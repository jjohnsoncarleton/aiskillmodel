...
# sai, sp
param1 = np.array([[0.5*smax, 0.75*smax],[1.5*smax, 1.25*smax]])
...
for i in range(paramshape[0]):
    ...
    for j in range(len(wss)):
        ws =wss[j]
        print((wd/ws*sp/smax-(wd-ws)/ws*sai/smax-ws)/(sai-smax))
        print(cmath.sqrt(a*g*M*(M-Pmax)*(sai-sp)*(sp-smax)/(sai-smax)))
